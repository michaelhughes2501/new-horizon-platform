...
**Wiring:** Added as a Vite dev-server plugin in vite.config.ts (gateway/vite-plugin-gateway.js).
**Dev base URL:** `http://localhost:5173`
**Key store:** `gateway.json in the project root`
...
curl -X POST http://localhost:5173/gateway/admin/keys \
...
curl http://localhost:5173/gateway/v1/ping -H "X-API-Key: <key>"
```
